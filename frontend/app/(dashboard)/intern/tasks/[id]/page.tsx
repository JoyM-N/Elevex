...
import { getInternTask } from '@/lib/api/intern/tasks'
import {
  TaskPriorityBadge,
  TaskStatusBadge,
  TaskTypeBadge,
} from '@/components/admin/tasks/badges'
import { CompleteTaskForm } from '@/components/intern/tasks/complete-task-form'
...
export default function InternTaskDetailPage() {
  ...
  const { data: task, isLoading, isError, refetch } = useQuery({
    queryKey: ['intern', 'tasks', id],
    queryFn: () => getInternTask(id),
    ...
    <div className="page-enter space-y-6">
      <div>
        ...
        <div className="mt-2 flex flex-wrap items-center gap-2">
          ...
          />
          <TaskTypeBadge type={task.task_type} label={task.task_type_label} />
          {task.is_overdue ? (
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <div className="rounded-xl bg-card p-4 ring-1 ring-border/80 lg:col-span-2">
        ...
        {!task.is_terminal ? <CompleteTaskForm taskId={task.id} /> : null}
      </div>
    </div>
  )
}
